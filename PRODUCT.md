...
- load a custom 3D model (drag/drop or file input, `.obj`/`.mtl`); no default model ships in the repo currently — the built-in rotating cube is shown until one is loaded
...
- Unrelated prototype files (`card.html`, `card-scramble.html`, `scroll-gradient.html`, `files/icon-mosaic-handover.md`, the `icons/` set, and the sample `3Dmodel/` asset) were removed from the repo as unused by this surface's actual code path.
...
- No NGEN 3D model asset currently ships in the repo — one must be sourced and loaded via the app's file input before a real demo.
...
4. NGEN's real model is the primary asset once loaded; the built-in placeholder cube geometry is a dev aid, not the shipped experience.
